...
const coresOriginais = new Map();
    ...
    function adaptarSiteParaDaltonismo() {
      const tipo = document.getElementById("daltonismo").value;
      const todos = document.querySelectorAll("*");

      todos.forEach(el => {
        const styles = window.getComputedStyle(el);

        // Salvar cores originais uma vez
        ...
        // Pegar as cores reais salvas
        const original = coresOriginais.get(el);
        let novaBg = original.backgroundColor;
        let novaColor = original.color;

        // Se for tipo diferente de "normal", aplicar simulação
        if (tipo !== "normal") {
          if (novaBg && novaBg !== "rgba(0, 0, 0, 0)" && novaBg !== "transparent") {
            novaBg = simularCor(novaBg, tipo);
          }
          if (novaColor && novaColor !== "rgba(0, 0, 0, 0)" && novaColor !== "transparent") {
            novaColor = simularCor(novaColor, tipo);
          }
        }

        // Aplicar ao elemento
        el.style.backgroundColor = novaBg;
        el.style.color = novaColor;
      });
    }

    function simularCor(rgb, tipo) {
      const [r, g, b] = extrairRGB(rgb);
      let novoR = r, novoG = g, novoB = b;
